...
def test_clean_and_transform(mock_df):
    ...
    assert cleaned['preco'].tolist() == [0.0, 1000.0]  # Invalido -> 0, 200 USD *5 = 1000
...
def test_generate_dashboard(mocker):
    ...
    mock_result_ebay.fetchall.return_value = mock_df_ebay.to_records(index=False).tolist()
...
def test_run_etl(mocker):
    ...
    mock_fetch = mocker.patch('etl.fetch_data', side_effect=[[{"id": "1"}], [{"id": "2"}]])
    ...
    assert mock_dashboard.call_count == 1  # Agora usa o mock_dashboard que foi retornado